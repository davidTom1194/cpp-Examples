...
TEST_CASE("Bubble Sort Test", "[sorting]") {
...
TEST_CASE("Sequential Search Test", "[searching]") {
    std::vector<int> vec = {5, 2, 9, 1, 5};
    auto result = seq_search(vec.begin(), vec.end(), 9);
    REQUIRE(result != vec.end());
    REQUIRE(*result == 9);

    result = seq_search(vec.begin(), vec.end(), 3);
    REQUIRE(result == vec.end());
}

TEST_CASE("Binary Search Test", "[searching]") {
    std::vector<int> vec = {1, 2, 5, 5, 9};
    auto result = bin_search(vec.begin(), vec.end(), 5);
    REQUIRE(result != vec.end());
    REQUIRE(*result == 5);

    result = bin_search(vec.begin(), vec.end(), 3);
    REQUIRE(result == vec.end());
}
